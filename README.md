...
Ascii-art Web is a project that involves hostings webpages for generate Ascii-Art of a desired text with their desired banner. You are presented an area to type text and only characters among the printable ASCII character range are displayed this includes,numerics and symbols.
...
After installation, you can access the web application by navigating to [http://localhost:8080](http://localhost:8080)
...
- Bad Requests: They occur when
  ...
  - The user gives an input that is not within the printable ASCII character range (from space to tilde `~`). There are 95 characters in total
...
Once you run the program a localhost server runs in the port provided. When you open the localhost provided you are directed to a page which resembles the image below.

![alt text](assets/images/input.png)

in the text box you enter the words you would like to be turned into word art. Then below it there is a dropdown option which allows you to choose the type of bannerfile you can use.
...
Once you have selected the banner file you can click on the submit button to get your word art.

A get request is sent to the server which intern takes the word type in the text area and the bannerfile selected as the inputs. When the program is run the following variations of processes occur:

### Case 1: Print non-printable character.

If the character doesn't range between the ascii values 32-126 and is not '\n' or '\t' then it is considered as a bad request error and an error message is printed out.
...
If the character is printable then a POST request is sent to the server to return the result of the ascii program to the output textbox.
...
Handles the styling used for the home, about and instructions page. Here is an overview of how this was achieved.
...
- Layout and Spacing: The main content wrapper is centered with a max-width of 1100px, a flexible width, and padding. It has a flex-column layout, making it easy to stack elements vertically.
